                ...
                <DropdownMenu icon={<FolderOpen size={20} />} label="Builds" color="indigo" title="Manage Scenes & Saves">
                    ...
                    <div className="max-h-[30vh] overflow-y-auto px-1 flex flex-col gap-1">
                        {filteredBuilds.map((model, idx) => (
                            <DropdownItem key={idx} onClick={() => props.onSelectCustomBuild(model)} icon={<History size={16} className="text-slate-400" />} label={model.name} truncate title={`Load "${model.name}"`} />
                        ))}
                    </div>
                </DropdownMenu>
...
const DropdownItem: React.FC<{onClick: () => void, icon: React.ReactNode, label: string, highlight?: boolean, truncate?: boolean, title?: string}> = ({ onClick, icon, label, highlight, truncate, title }) => (
    <button onClick={() => { Sound.play('ui'); onClick(); }} title={title} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${highlight ? 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100' : 'text-slate-600 hover:bg-slate-50'}`} onMouseEnter={() => Sound.play('hover')}>
