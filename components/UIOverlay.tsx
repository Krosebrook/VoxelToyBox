...
import { 
  Box, Bird, History, Play, Pause, 
  Pencil, Eraser, Pipette, Palette, Undo2, Redo2, Camera, 
  Sparkles, Zap, BoxSelect, PlusCircle, Save, Trash2, Copy,
...
// --- Atomic Sub-Components (Defined first to avoid ReferenceErrors) ---
...
const TactileButton: React.FC<{onClick: () => void, icon: React.ReactNode, label?: string, color: string, compact?: boolean, title?: string}> = ({ onClick, icon, label, color, compact, title }) => {
    const bg = ({ sky: 'bg-sky-500 hover:bg-sky-600', emerald: 'bg-emerald-500 hover:bg-emerald-600', indigo: 'bg-indigo-500 hover:bg-indigo-600', slate: 'bg-slate-700 hover:bg-slate-800', amber: 'bg-amber-500 hover:bg-amber-600' } as any)[color] || 'bg-slate-500';
    return (
...
const BigActionButton: React.FC<{onClick: () => void, icon: React.ReactNode, label: string, color: string, glow?: boolean, title?: string}> = ({ onClick, icon, label, color, glow, title }) => {
    const bg = ({ rose: 'bg-rose-500 border-rose-800 hover:bg-rose-600', indigo: 'bg-indigo-500 border-indigo-800 hover:bg-indigo-600' } as any)[color] || 'bg-slate-500';
    return (
...
const DropdownItem: React.FC<{onClick: (e: React.MouseEvent) => void, icon: React.ReactNode, label: string, highlight?: boolean, truncate?: boolean, title?: string, onDelete?: (e: React.MouseEvent) => void}> = ({ onClick, icon, label, highlight, truncate, title, onDelete }) => (
    <button 
        onClick={onClick} 
        title={title}
        className={`w-full flex items-center justify-between p-2 rounded-xl transition-all hover:bg-slate-100 group/item ${highlight ? 'bg-indigo-50/50' : ''}`}
    >
        <div className="flex items-center gap-3 overflow-hidden">
            <div className={`shrink-0 ${highlight ? 'text-indigo-600' : 'text-slate-500'}`}>{icon}</div>
            <span className={`text-sm font-bold text-slate-700 ${truncate ? 'truncate' : ''}`}>{label}</span>
        </div>
        {onDelete && (
            <div 
                onClick={onDelete}
                className="p-1.5 opacity-0 group-hover/item:opacity-100 hover:bg-rose-100 text-slate-400 hover:text-rose-500 rounded-lg transition-all"
            >
                <Trash2 size={14} />
            </div>
        )}
...
const DropdownMenu: React.FC<{icon: React.ReactNode, label: string, color: string, children: React.ReactNode, title?: string}> = ({ icon, label, color, children, title }) => {
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {open && (
                <div className="absolute left-0 top-full mt-4 w-64 bg-white/95 backdrop-blur-md border border-slate-200 shadow-2xl rounded-2xl p-2 z-50 flex flex-col gap-1 animate-in fade-in slide-in-from-top-2 pointer-events-auto">
                    {children}
                </div>
            )}
        </div>
        ...
        <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
                <DropdownMenu icon={<FolderOpen size={20} />} label="Builds" color="indigo" title="Manage Scenes & Saves">
                    ...
                    <DropdownItem onClick={props.onLoadRecentBuild} icon={<FileUp size={16} className="text-sky-500" />} label="Load Build" title="Load most recently saved build" />
                    ...
                    {filteredBuilds.length === 0 && (
                        <div className="px-3 py-2 text-xs text-slate-400 italic text-center">No saved builds yet</div>
                    )}
          ...
          <div className="absolute bottom-24 left-1/2 -translate-x-1/2 pointer-events-auto bg-white/90 backdrop-blur-xl border border-slate-200 shadow-2xl rounded-3xl p-4 flex flex-col gap-4 animate-in slide-in-from-bottom-8">
              ...
              <div className="flex flex-col items-center gap-2 border-b border-slate-100 pb-3">
               ...
               <div className="flex justify-center gap-2 border-b border-slate-100 pb-3">
                   <button 
                       onClick={props.onUndoSelection} 
                       disabled={!props.canUndoSelection}
                       className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${props.canUndoSelection ? 'bg-slate-100 hover:bg-slate-200 text-slate-600' : 'bg-slate-50 text-slate-300 cursor-not-allowed'}`}
                       title="Undo Selection Change"
                   >
                       <Undo2 size={14} /> Undo Action
                   </button>
                   <button 
                       onClick={props.onRedoSelection} 
                       disabled={!props.canRedoSelection}
                       className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${props.canRedoSelection ? 'bg-slate-100 hover:bg-slate-200 text-slate-600' : 'bg-slate-50 text-slate-300 cursor-not-allowed'}`}
                       title="Redo Selection Change"
                   >
                       <Redo2 size={14} /> Redo Action
                   </button>
               </div>
